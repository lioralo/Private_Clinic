...
### Tests & lint
- Test command is documented above (`python -m pytest tests/ -q`, with `SECRET_KEY` / `FLASK_ENV` / `TESTING` set). The harness uses `tests/db_test_support.py` (`build_test_schema`) which applies Alembic via `upgrade heads` into a cached template DB — expect a green suite (a few intentional skips).
- Logout is POST-only; any new tests must POST `/logout` (not GET).
- Lint-ish checks: `python verification/refactor_guard.py check` (route contract). Optional style: `ruff check` (`ruff.toml` is present; `ruff` is not in `requirements.txt`, install ad hoc if needed).
- Optional integrations (Google OAuth/Docs/Calendar, SMTP/SES, Twilio) are not required for local core clinic flows; leave those env vars unset.
